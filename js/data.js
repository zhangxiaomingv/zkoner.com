...
function emptyMonitorData() {
  return {
    meta: { updated_at: '--', generated_by: '暂无真实数据', brand: '优引GEO系统', website: '', data_version: '0.1.0' },
    settings: { brand: {}, engines: [], scenarios: [] },
    visibility: { overall_score: null, score_delta: null, mentioned_scenarios: 0, total_scenarios: 0, history: [], latest: { engines: [] } },
    competitors: { updated_at: '', list: [] },
    citations: [],
    articles: [],
    scenario_insights: [],
    content_tracking: [],
...
const DataStore = {
  ...
  async load() {
    ...
    this.data = emptyMonitorData();
    this.source = 'offline (无真实数据)';
    return false;
  },
  ...
  notice() {
    const m = this.data.meta || {};
    if (this.source.includes('offline') || this.source.includes('示例')) {
      return { type: 'demo', text: '当前没有真实监测数据。请确认云端监测已运行，并登录后重试。' };
    }
